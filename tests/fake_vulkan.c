...
static const VkDescriptorUpdateTemplate fake_dxvk_descriptor_template =
    (VkDescriptorUpdateTemplate)(uintptr_t)UINT64_C(0xab00);
static const VkDescriptorPool fake_dxvk_descriptor_pool =
...
static const VkDescriptorSet fake_dxvk_descriptor_set_2 =
    (VkDescriptorSet)(uintptr_t)UINT64_C(0xad01);
...
static VkResult VKAPI_CALL fake_create_descriptor_set_layout(
    ...
    if (fake_descriptor_template_step == 0U && allocator == NULL &&
        create_info != NULL && set_layout != NULL &&
        create_info->sType ==
            VK_STRUCTURE_TYPE_DESCRIPTOR_SET_LAYOUT_CREATE_INFO &&
        create_info->pNext == NULL && create_info->flags == 0U &&
        create_info->bindingCount == 4U && create_info->pBindings != NULL) {
        static const VkDescriptorType dxvk_types[] = {
            VK_DESCRIPTOR_TYPE_STORAGE_BUFFER,
            VK_DESCRIPTOR_TYPE_STORAGE_BUFFER,
            VK_DESCRIPTOR_TYPE_UNIFORM_TEXEL_BUFFER,
            VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE,
        };
        static const VkShaderStageFlags dxvk_stages[] = {
        ...
        for (uint32_t index = 0U; index < 4U; ++index) {
            ...
            if (binding->binding != index ||
                binding->descriptorType != dxvk_types[index] ||
                binding->descriptorCount != 1U ||
                binding->stageFlags != dxvk_stages[index] ||
                binding->pImmutableSamplers != NULL) {
                return VK_ERROR_INITIALIZATION_FAILED;
            }
        }
        *set_layout = fake_dxvk_descriptor_layout;
        fake_descriptor_template_step = 1U;
        return VK_SUCCESS;
    }
...
static VkResult VKAPI_CALL fake_create_descriptor_pool(
    ...
    if (fake_descriptor_template_step == 2U && allocator == NULL &&
        create_info != NULL && pool != NULL &&
        create_info->sType == VK_STRUCTURE_TYPE_DESCRIPTOR_POOL_CREATE_INFO &&
        create_info->pNext == NULL && create_info->flags == 0U &&
        create_info->maxSets == (require_journal ? 2U : 1U) &&
        create_info->poolSizeCount == 3U &&
        create_info->pPoolSizes != NULL &&
        create_info->pPoolSizes[0].type == VK_DESCRIPTOR_TYPE_STORAGE_BUFFER &&
        create_info->pPoolSizes[0].descriptorCount ==
            (require_journal ? 4U : 2U) &&
        create_info->pPoolSizes[1].type ==
            VK_DESCRIPTOR_TYPE_UNIFORM_TEXEL_BUFFER &&
        create_info->pPoolSizes[1].descriptorCount ==
            (require_journal ? 2U : 1U) &&
        create_info->pPoolSizes[2].type == VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE &&
        create_info->pPoolSizes[2].descriptorCount ==
            (require_journal ? 2U : 1U)) {
        *pool = fake_dxvk_descriptor_pool;
        fake_descriptor_template_step = 3U;
        return VK_SUCCESS;
    }
    if (fake_descriptor_step == 0U && allocator == NULL &&
        create_info != NULL && pool != NULL &&
...
static VkResult VKAPI_CALL fake_allocate_descriptor_sets(
    ...
    if (getenv("BVB_FAKE_REQUIRE_DESCRIPTOR_JOURNAL") != NULL &&
        fake_descriptor_template_step == 5U && allocate_info != NULL &&
        descriptor_sets != NULL &&
        allocate_info->sType == VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO &&
        allocate_info->pNext == NULL &&
        allocate_info->descriptorPool == fake_dxvk_descriptor_pool &&
        ...
        allocate_info->pSetLayouts != NULL &&
        allocate_info->pSetLayouts[0] == fake_dxvk_descriptor_layout) {
        descriptor_sets[0] = fake_dxvk_descriptor_set_2;
        fake_descriptor_transaction_allocate_seen = 1;
        return VK_SUCCESS;
    }
...
static VkResult VKAPI_CALL fake_create_descriptor_update_template(
    VkDevice device,
    const VkDescriptorUpdateTemplateCreateInfo *create_info,
    const VkAllocationCallbacks *allocator,
    VkDescriptorUpdateTemplate *descriptor_update_template) {
    (void)device;
    static const VkDescriptorType expected_types[] = {
        VK_DESCRIPTOR_TYPE_STORAGE_BUFFER,
        VK_DESCRIPTOR_TYPE_STORAGE_BUFFER,
        VK_DESCRIPTOR_TYPE_UNIFORM_TEXEL_BUFFER,
        VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE,
    };
    if (fake_descriptor_template_step != 1U || allocator != NULL ||
        create_info == NULL || descriptor_update_template == NULL ||
        create_info->sType !=
            VK_STRUCTURE_TYPE_DESCRIPTOR_UPDATE_TEMPLATE_CREATE_INFO ||
        create_info->pNext != NULL || create_info->flags != 0U ||
        create_info->descriptorUpdateEntryCount != 4U ||
        create_info->pDescriptorUpdateEntries == NULL ||
        create_info->templateType !=
            VK_DESCRIPTOR_UPDATE_TEMPLATE_TYPE_DESCRIPTOR_SET ||
        create_info->descriptorSetLayout != fake_dxvk_descriptor_layout ||
    ...
    for (uint32_t index = 0U; index < 4U; ++index) {
        ...
        if (entry->dstBinding != index || entry->dstArrayElement != 0U ||
            entry->descriptorCount != 1U ||
            entry->descriptorType != expected_types[index] ||
            entry->offset != 24U * index || entry->stride != 24U) {
            return VK_ERROR_INITIALIZATION_FAILED;
        }
    }
    *descriptor_update_template = fake_dxvk_descriptor_template;
    fake_descriptor_template_step = 2U;
    return VK_SUCCESS;
}

static void VKAPI_CALL fake_destroy_descriptor_update_template(
    VkDevice device, VkDescriptorUpdateTemplate descriptor_update_template,
    const VkAllocationCallbacks *allocator) {
    (void)device;
    if (fake_descriptor_template_step == 7U && allocator == NULL &&
        descriptor_update_template == fake_dxvk_descriptor_template) {
        fake_descriptor_template_step = 8U;
    }
}

static void VKAPI_CALL fake_update_descriptor_set_with_template(
    ...
    for (uint32_t index = 0U; index < 2U; ++index) {
        ...
        memcpy(&info, bytes + index * 24U, sizeof(info));
        if (info.buffer != VK_NULL_HANDLE || info.offset != 0U ||
            info.range != VK_WHOLE_SIZE) return;
    }
    VkBufferView view = (VkBufferView)(uintptr_t)1U;
    memcpy(&view, bytes + 48U, sizeof(view));
...
static void VKAPI_CALL fake_cmd_bind_descriptor_sets(
    ...
    if (command_buffer == VK_NULL_HANDLE ||
        ...
        layout != fake_pipeline_layout || first_set != 0U ||
        descriptor_set_count != 1U || descriptor_sets == NULL ||
        descriptor_sets[0] != fake_descriptor_set ||
...
static void VKAPI_CALL fake_destroy_descriptor_set_layout(
    ...
    if (fake_descriptor_template_step == 6U &&
        set_layout == fake_dxvk_descriptor_layout) {
        fake_descriptor_template_step = 7U;
        return;
    }
...
static VkResult VKAPI_CALL fake_device_wait_idle(VkDevice device) {
    ...
    if (getenv("BVB_FAKE_REQUIRE_DESCRIPTOR_JOURNAL") != NULL &&
        ...
        (fake_descriptor_journal_wait_seen != 1 ||
         fake_descriptor_transaction_allocate_seen != 1)) {
        fprintf(stderr,
                "fake descriptor transaction did not allocate before wait\n");
        return VK_ERROR_INITIALIZATION_FAILED;
    }
    if (fake_animation_enabled() &&
        (fake_animation_frame_count != 4U ||
...
#define BVB_DEVICE_MATCH(vulkan_name, fake_name)                                \
    ...
    BVB_DEVICE_MATCH("vkCreateDescriptorUpdateTemplate",
                     fake_create_descriptor_update_template)
    BVB_DEVICE_MATCH("vkDestroyDescriptorUpdateTemplate",
                     fake_destroy_descriptor_update_template)
