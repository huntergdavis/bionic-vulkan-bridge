...
static bool query_type_is_supported(uint32_t query_type) {
    return query_type == VK_QUERY_TYPE_OCCLUSION ||
           query_type == VK_QUERY_TYPE_PIPELINE_STATISTICS ||
           query_type == VK_QUERY_TYPE_TIMESTAMP ||
           query_type == VK_QUERY_TYPE_TRANSFORM_FEEDBACK_STREAM_EXT;
}
...
int bvb_protocol_encode_vulkan_query_pool_create_request(
    ...
    if (output == NULL || request == NULL ||
        ...
        request->flags != 0U || !query_type_is_supported(request->query_type) ||
        request->query_count == 0U || request->query_count > (1U << 20) ||
        (request->query_type == VK_QUERY_TYPE_PIPELINE_STATISTICS
             ? request->pipeline_statistics == 0U ||
                   (request->pipeline_statistics & ~UINT32_C(0x7ff)) != 0U
             : request->pipeline_statistics != 0U))
        return -EINVAL;
    memset(output, 0, BVB_VULKAN_QUERY_POOL_CREATE_REQUEST_SIZE);
    bvb_wire_put_u64(output, request->device_id);
    bvb_wire_put_u32(output + 8, request->flags);
    bvb_wire_put_u32(output + 12, request->query_type);
    bvb_wire_put_u32(output + 16, request->query_count);
    bvb_wire_put_u32(output + 20, request->pipeline_statistics);
    return 0;
}
...
int bvb_protocol_decode_vulkan_query_pool_create_request(
    ...
    const struct bvb_vulkan_query_pool_create_request decoded = {
        .device_id = bvb_wire_get_u64(input),
        .flags = bvb_wire_get_u32(input + 8),
        .query_type = bvb_wire_get_u32(input + 12),
        .query_count = bvb_wire_get_u32(input + 16),
        .pipeline_statistics = bvb_wire_get_u32(input + 20),
    };
    uint8_t validation[BVB_VULKAN_QUERY_POOL_CREATE_REQUEST_SIZE];
    if (bvb_protocol_encode_vulkan_query_pool_create_request(
...
int bvb_protocol_encode_vulkan_query_pool_results_request(
    ...
    if (output == NULL || request == NULL ||
        !wire_id_is_type(request->query_pool_id, BVB_OBJECT_QUERY_POOL) ||
        request->data_size == 0U ||
        request->data_size > BVB_VULKAN_QUERY_POOL_RESULTS_MAX_BYTES ||
        request->stride == 0U || request->stride > request->data_size ||
        request->query_count == 0U || request->query_count > 256U ||
        request->first_query > UINT32_MAX - request->query_count ||
        (request->flags & ~UINT32_C(0x1f)) != 0U)
        return -EINVAL;
    memset(output, 0, BVB_VULKAN_QUERY_POOL_RESULTS_REQUEST_SIZE);
    bvb_wire_put_u64(output, request->query_pool_id);
    bvb_wire_put_u64(output + 8, request->data_size);
    bvb_wire_put_u64(output + 16, request->stride);
    bvb_wire_put_u32(output + 24, request->first_query);
    bvb_wire_put_u32(output + 28, request->query_count);
    bvb_wire_put_u32(output + 32, request->flags);
    return 0;
}
...
int bvb_protocol_decode_vulkan_query_pool_results_request(
    ...
    const struct bvb_vulkan_query_pool_results_request decoded = {
        .query_pool_id = bvb_wire_get_u64(input),
        .data_size = bvb_wire_get_u64(input + 8),
        .stride = bvb_wire_get_u64(input + 16),
        .first_query = bvb_wire_get_u32(input + 24),
        .query_count = bvb_wire_get_u32(input + 28),
        .flags = bvb_wire_get_u32(input + 32),
    };
    uint8_t validation[BVB_VULKAN_QUERY_POOL_RESULTS_REQUEST_SIZE];
    if (bvb_protocol_encode_vulkan_query_pool_results_request(
            validation, &decoded) != 0 ||
        memcmp(validation, input, sizeof(validation)) != 0)
        return -EPROTO;
    *request = decoded;
...
int bvb_protocol_encode_vulkan_query_pool_results_response(
    ...
    if (output == NULL || response == NULL || output_length == NULL ||
        ...
        response->data_size > BVB_VULKAN_QUERY_POOL_RESULTS_MAX_BYTES)
        return -EINVAL;
    ...
    memset(output, 0, length);
    bvb_wire_put_i32(output, response->vulkan_result);
    bvb_wire_put_u32(output + 4, response->data_size);
    memcpy(output + BVB_VULKAN_QUERY_POOL_RESULTS_RESPONSE_PREFIX_SIZE,
...
int bvb_protocol_decode_vulkan_query_pool_results_response(
    ...
    if (input == NULL || response == NULL ||
        input_length < BVB_VULKAN_QUERY_POOL_RESULTS_RESPONSE_PREFIX_SIZE)
        return -EINVAL;
    const uint32_t data_size = bvb_wire_get_u32(input + 4);
    if (data_size == 0U ||
    ...
    *response = (struct bvb_vulkan_query_pool_results_response){
    ...
    memcpy(response->data,
           input + BVB_VULKAN_QUERY_POOL_RESULTS_RESPONSE_PREFIX_SIZE,
           data_size);
    return 0;
}
...
int bvb_protocol_encode_vulkan_query_pool_reset_request(
    ...
    if (output == NULL || request == NULL ||
        !wire_id_is_type(request->query_pool_id, BVB_OBJECT_QUERY_POOL) ||
        request->query_count == 0U ||
        request->first_query > UINT32_MAX - request->query_count)
        return -EINVAL;
    bvb_wire_put_u64(output, request->query_pool_id);
    bvb_wire_put_u32(output + 8, request->first_query);
    bvb_wire_put_u32(output + 12, request->query_count);
    return 0;
}
...
int bvb_protocol_decode_vulkan_query_pool_reset_request(
    ...
    const struct bvb_vulkan_query_pool_reset_request decoded = {
        .query_pool_id = bvb_wire_get_u64(input),
        .first_query = bvb_wire_get_u32(input + 8),
        .query_count = bvb_wire_get_u32(input + 12),
    };
    uint8_t validation[BVB_VULKAN_QUERY_POOL_RESET_REQUEST_SIZE];
    if (bvb_protocol_encode_vulkan_query_pool_reset_request(
            validation, &decoded) != 0 ||
        memcmp(validation, input, sizeof(validation)) != 0)
        return -EPROTO;
    *request = decoded;
...
static bool buffer_requirements_2_pnext_valid(uint32_t pnext_flags) {
    return (pnext_flags &
            ~BVB_VULKAN_BUFFER_REQUIREMENTS_2_PNEXT_MASK) == 0U;
}

int bvb_protocol_encode_vulkan_buffer_requirements_2_request(
    uint8_t output[BVB_VULKAN_BUFFER_REQUIREMENTS_2_REQUEST_SIZE],
    const struct bvb_vulkan_buffer_requirements_2_request *request) {
    if (output == NULL || request == NULL ||
        !wire_id_is_type(request->buffer_id, BVB_OBJECT_BUFFER) ||
        !buffer_requirements_2_pnext_valid(request->pnext_flags)) {
        return -EINVAL;
    }
    memset(output, 0, BVB_VULKAN_BUFFER_REQUIREMENTS_2_REQUEST_SIZE);
    bvb_wire_put_u64(output, request->buffer_id);
    bvb_wire_put_u32(output + 8, request->pnext_flags);
    return 0;
}

int bvb_protocol_decode_vulkan_buffer_requirements_2_request(
    const uint8_t input[BVB_VULKAN_BUFFER_REQUIREMENTS_2_REQUEST_SIZE],
    struct bvb_vulkan_buffer_requirements_2_request *request) {
    if (input == NULL || request == NULL) return -EINVAL;
    ...
    if (!wire_id_is_type(decoded.buffer_id, BVB_OBJECT_BUFFER) ||
        !buffer_requirements_2_pnext_valid(decoded.pnext_flags) ||
        bvb_wire_get_u32(input + 12) != 0U) {
        return -EPROTO;
    }
    *request = decoded;
...
int bvb_protocol_encode_vulkan_buffer_requirements_2_response(
    uint8_t output[BVB_VULKAN_BUFFER_REQUIREMENTS_2_RESPONSE_SIZE],
    const struct bvb_vulkan_buffer_requirements_2_response *response) {
    if (output == NULL || response == NULL || response->size == 0U ||
        response->alignment == 0U || response->memory_type_bits == 0U ||
        !buffer_requirements_2_pnext_valid(response->pnext_flags) ||
...
int bvb_protocol_decode_vulkan_buffer_requirements_2_response(
    const uint8_t input[BVB_VULKAN_BUFFER_REQUIREMENTS_2_RESPONSE_SIZE],
    struct bvb_vulkan_buffer_requirements_2_response *response) {
    if (input == NULL || response == NULL) return -EINVAL;
    const struct bvb_vulkan_buffer_requirements_2_response decoded = {
        .size = bvb_wire_get_u64(input),
        .alignment = bvb_wire_get_u64(input + 8),
        .memory_type_bits = bvb_wire_get_u32(input + 16),
    ...
    uint8_t canonical[BVB_VULKAN_BUFFER_REQUIREMENTS_2_RESPONSE_SIZE];
    if (bvb_protocol_encode_vulkan_buffer_requirements_2_response(
            canonical, &decoded) != 0) {
        return -EPROTO;
    }
    *response = decoded;
...
int bvb_protocol_encode_vulkan_buffer_device_address_request(
    uint8_t output[BVB_VULKAN_BUFFER_DEVICE_ADDRESS_REQUEST_SIZE],
...
int bvb_protocol_decode_vulkan_buffer_device_address_request(
    const uint8_t input[BVB_VULKAN_BUFFER_DEVICE_ADDRESS_REQUEST_SIZE],
    struct bvb_vulkan_buffer_device_address_request *request) {
    if (input == NULL || request == NULL) return -EINVAL;
...
int bvb_protocol_encode_vulkan_buffer_device_address_response(
    uint8_t output[BVB_VULKAN_BUFFER_DEVICE_ADDRESS_RESPONSE_SIZE],
    const struct bvb_vulkan_buffer_device_address_response *response) {
    if (output == NULL || response == NULL ||
        response->device_address == 0U) {
        return -EINVAL;
    }
    bvb_wire_put_u64(output, response->device_address);
    return 0;
}

int bvb_protocol_decode_vulkan_buffer_device_address_response(
    const uint8_t input[BVB_VULKAN_BUFFER_DEVICE_ADDRESS_RESPONSE_SIZE],
    struct bvb_vulkan_buffer_device_address_response *response) {
    if (input == NULL || response == NULL) return -EINVAL;
    const struct bvb_vulkan_buffer_device_address_response decoded = {
        .device_address = bvb_wire_get_u64(input),
    };
    if (decoded.device_address == 0U) return -EPROTO;
    *response = decoded;
    return 0;
}
...
static int encode_vulkan_queue_submit_2_request(
    ...
    bvb_wire_put_u32(output + 16, request->flags);
    bvb_wire_put_u32(output + 20, request->wait_count);
    ...
    for (uint32_t index = 0U; index < request->wait_count; ++index) {
        const struct bvb_vulkan_submit_2_semaphore_record *record =
            &request->waits[index];
        if (!wire_id_is_type(record->semaphore_id, 17U)) return -EINVAL;
        bvb_wire_put_u64(output + offset, record->semaphore_id);
        bvb_wire_put_u64(output + offset + 8, record->value);
        bvb_wire_put_u64(output + offset + 16, record->stage_mask);
        bvb_wire_put_u32(output + offset + 24, record->device_index);
...
static int decode_vulkan_queue_submit_2_request(
    ...
    const uint32_t command_count = bvb_wire_get_u32(input + 24);
    const uint32_t signal_count = bvb_wire_get_u32(input + 28);
    ...
    *request = (struct bvb_vulkan_queue_submit_2_request){
        .queue_id = bvb_wire_get_u64(input),
        .fence_id = bvb_wire_get_u64(input + 8),
        .flags = bvb_wire_get_u32(input + 16),
        .wait_count = wait_count,
        .command_count = command_count,
        .signal_count = signal_count,
    };
    if (!wire_id_is_type(request->queue_id, 4U) ||
        (request->fence_id != 0U &&
    ...
    for (uint32_t index = 0U; index < wait_count; ++index) {
        request->waits[index] =
            (struct bvb_vulkan_submit_2_semaphore_record){
                .semaphore_id = bvb_wire_get_u64(input + offset),
                .value = bvb_wire_get_u64(input + offset + 8),
                .stage_mask = bvb_wire_get_u64(input + offset + 16),
                .device_index = bvb_wire_get_u32(input + offset + 24),
            };
        if (!wire_id_is_type(request->waits[index].semaphore_id, 17U) ||
            bvb_wire_get_u32(input + offset + 28) != 0U) return -EPROTO;
        offset += BVB_VULKAN_SUBMIT_2_SEMAPHORE_RECORD_SIZE;
    }
    ...
    for (uint32_t index = 0U; index < signal_count; ++index) {
        ...
        if (!wire_id_is_type(request->signals[index].semaphore_id, 17U) ||
            bvb_wire_get_u32(input + offset + 28) != 0U) return -EPROTO;
        offset += BVB_VULKAN_SUBMIT_2_SEMAPHORE_RECORD_SIZE;
    }
    return 0;
...
int bvb_protocol_encode_vulkan_swapchain_acquire_request(
    ...
    if (output == NULL || request == NULL ||
        !wire_id_is_type(request->device_id, BVB_OBJECT_DEVICE) ||
        !wire_id_is_type(request->swapchain_id, BVB_OBJECT_SWAPCHAIN) ||
        (request->semaphore_id == 0U && request->fence_id == 0U) ||
        (request->semaphore_id != 0U &&
         !wire_id_is_type(request->semaphore_id, BVB_OBJECT_SEMAPHORE)) ||
        (request->fence_id != 0U &&
         !wire_id_is_type(request->fence_id, BVB_OBJECT_FENCE))) {
        return -EINVAL;
    }
    memset(output, 0, BVB_VULKAN_SWAPCHAIN_ACQUIRE_REQUEST_SIZE);
    bvb_wire_put_u64(output, request->device_id);
    bvb_wire_put_u64(output + 8, request->swapchain_id);
    bvb_wire_put_u64(output + 16, request->timeout_ns);
    bvb_wire_put_u64(output + 24, request->semaphore_id);
    bvb_wire_put_u64(output + 32, request->fence_id);
    return 0;
}
...
int bvb_protocol_decode_vulkan_swapchain_acquire_request(
    ...
    const struct bvb_vulkan_swapchain_acquire_request decoded = {
        .device_id = bvb_wire_get_u64(input),
        .swapchain_id = bvb_wire_get_u64(input + 8),
        .timeout_ns = bvb_wire_get_u64(input + 16),
        .semaphore_id = bvb_wire_get_u64(input + 24),
        .fence_id = bvb_wire_get_u64(input + 32),
    };
    uint8_t validation[BVB_VULKAN_SWAPCHAIN_ACQUIRE_REQUEST_SIZE];
    if (bvb_protocol_encode_vulkan_swapchain_acquire_request(
            validation, &decoded) != 0) return -EPROTO;
    *request = decoded;
    return 0;
}
...
int bvb_protocol_encode_vulkan_swapchain_acquire_response(
    ...
    if (output == NULL || response == NULL ||
        (response->vulkan_result == 0 &&
         response->image_index >= BVB_WSI_FRAME_RING_MAX_SLOTS) ||
        (response->vulkan_result != 0 &&
         response->image_index != 0U)) return -EINVAL;
    bvb_wire_put_i32(output, response->vulkan_result);
    bvb_wire_put_u32(output + 4, response->image_index);
    return 0;
}
...
int bvb_protocol_decode_vulkan_swapchain_acquire_response(
    ...
    if (input == NULL || response == NULL) return -EINVAL;
    const struct bvb_vulkan_swapchain_acquire_response decoded = {
        .vulkan_result = bvb_wire_get_i32(input),
        .image_index = bvb_wire_get_u32(input + 4),
    };
    uint8_t validation[BVB_VULKAN_SWAPCHAIN_ACQUIRE_RESPONSE_SIZE];
    if (bvb_protocol_encode_vulkan_swapchain_acquire_response(
            validation, &decoded) != 0) return -EPROTO;
    *response = decoded;
    return 0;
}
...
int bvb_protocol_encode_vulkan_swapchain_present_request(
    ...
    uint32_t *output_length) {
    if (output == NULL || request == NULL || output_length == NULL ||
        !wire_id_is_type(request->queue_id, BVB_OBJECT_QUEUE) ||
        !wire_id_is_type(request->swapchain_id, BVB_OBJECT_SWAPCHAIN) ||
        request->image_index >= BVB_WSI_FRAME_RING_MAX_SLOTS ||
        request->wait_semaphore_count >
    ...
    for (uint32_t index = 0U; index < request->wait_semaphore_count; ++index) {
        if (!wire_id_is_type(request->wait_semaphore_ids[index],
                             BVB_OBJECT_SEMAPHORE)) return -EINVAL;
    }
    const uint32_t length = BVB_VULKAN_SWAPCHAIN_PRESENT_PREFIX_SIZE +
        request->wait_semaphore_count * sizeof(uint64_t);
    memset(output, 0, length);
    bvb_wire_put_u64(output, request->queue_id);
    bvb_wire_put_u64(output + 8, request->swapchain_id);
    bvb_wire_put_u32(output + 16, request->image_index);
    bvb_wire_put_u32(output + 20, request->wait_semaphore_count);
    bvb_wire_put_u32(output + 24, request->flags);
    for (uint32_t index = 0U; index < request->wait_semaphore_count; ++index) {
        bvb_wire_put_u64(output + BVB_VULKAN_SWAPCHAIN_PRESENT_PREFIX_SIZE +
                             index * sizeof(uint64_t),
                         request->wait_semaphore_ids[index]);
    }
    *output_length = length;
    return 0;
}
...
int bvb_protocol_decode_vulkan_swapchain_present_request(
    ...
    if (input == NULL || request == NULL ||
        input_length < BVB_VULKAN_SWAPCHAIN_PRESENT_PREFIX_SIZE ||
        bvb_wire_get_u32(input + 28) != 0U) return -EINVAL;
    struct bvb_vulkan_swapchain_present_request decoded = {
        .queue_id = bvb_wire_get_u64(input),
        .swapchain_id = bvb_wire_get_u64(input + 8),
        .image_index = bvb_wire_get_u32(input + 16),
        .wait_semaphore_count = bvb_wire_get_u32(input + 20),
        .flags = bvb_wire_get_u32(input + 24),
    };
    if (decoded.wait_semaphore_count >
            BVB_VULKAN_MAX_PRESENT_WAIT_SEMAPHORES ||
        input_length != BVB_VULKAN_SWAPCHAIN_PRESENT_PREFIX_SIZE +
                            decoded.wait_semaphore_count * sizeof(uint64_t)) {
        return -EPROTO;
    }
    for (uint32_t index = 0U; index < decoded.wait_semaphore_count; ++index) {
        decoded.wait_semaphore_ids[index] = bvb_wire_get_u64(
            input + BVB_VULKAN_SWAPCHAIN_PRESENT_PREFIX_SIZE +
            index * sizeof(uint64_t));
    }
    uint8_t validation[BVB_VULKAN_SWAPCHAIN_PRESENT_MAX_SIZE];
    uint32_t validation_length = 0U;
    if (bvb_protocol_encode_vulkan_swapchain_present_request(
            validation, &decoded, &validation_length) != 0 ||
        validation_length != input_length) return -EPROTO;
    *request = decoded;
    return 0;
}
...
int bvb_protocol_encode_vulkan_swapchain_present_response(
    ...
    if (output == NULL || response == NULL ||
        (response->vulkan_result == 0 && response->sequence == 0U) ||
        (response->vulkan_result != 0 && response->sequence != 0U))
        return -EINVAL;
    bvb_wire_put_i32(output, response->vulkan_result);
    bvb_wire_put_u32(output + 4, response->sequence);
    return 0;
}
...
int bvb_protocol_decode_vulkan_swapchain_present_response(
    ...
    if (input == NULL || response == NULL) return -EINVAL;
    const struct bvb_vulkan_swapchain_present_response decoded = {
        .vulkan_result = bvb_wire_get_i32(input),
        .sequence = bvb_wire_get_u32(input + 4),
    };
    uint8_t validation[BVB_VULKAN_SWAPCHAIN_PRESENT_RESPONSE_SIZE];
    if (bvb_protocol_encode_vulkan_swapchain_present_response(
            validation, &decoded) != 0) return -EPROTO;
    *response = decoded;
    return 0;
}
