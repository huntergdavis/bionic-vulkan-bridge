...
int main(int argc, char **argv) {
    ...
    int external_fd = -1;
    if (result == 0) {
        result = bvb_transport_receive_fd(connection, &request, &external_fd);
    }
    ...
    if (result == 0) {
        result = bvb_vulkan_batch_context_import_external_memory_fd(
            context, external_fd, import_request.allocation_size,
    ...
    printf("{\"schema_version\":1,\"gate\":\"E036\","
           ...
           import_request.allocation_size, import_result.memory_type_index,
           import_result.memory_property_flags, import_result.buffer_bytes,
           import_result.mismatched_bytes,
