...
helper_apk="$out_dir/e041-shared-region-client.apk"
receiver="$build_dir/bvb-external-memory-receiver"
control_client="$build_dir/bvb-bridge-client"

for command_name in am aapt cmake cp env grep mktemp od pm python3 readelf \
    sed sleep tr; do
...
runtime_dir=$(mktemp -d "${TMPDIR:-/tmp}/bvb-e041.XXXXXX")
control_socket="$runtime_dir/bridge.sock"
socket_name="bvb-e041-$(od -An -N8 -tx1 /dev/urandom | tr -d ' \n')"
token=$(od -An -N32 -tx1 /dev/urandom | tr -d ' \n')
wrong_token=ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff
service_stdout="$out_dir/e041-service.stdout"
service_stderr="$out_dir/e041-service.stderr"
receiver_stdout="$out_dir/e041-receiver.stdout"
receiver_stderr="$out_dir/e041-receiver.stderr"
wrong_json="$out_dir/e041-wrong-token.json"
wrong_stderr="$out_dir/e041-wrong-token.stderr"
valid_json="$out_dir/e041-valid-token.json"
valid_stderr="$out_dir/e041-valid-token.stderr"
status_json="$out_dir/e041-activity-status.json"
evidence="$out_dir/e041-fenced-external-image.json"
service_pid=
receiver_pid=
cleanup() {
...
while ! grep -q '^bvb-external-memory-receiver: ready ' "$receiver_stdout"; do
    kill -0 "$receiver_pid" 2>/dev/null || {
        printf 'fenced-image receiver exited before readiness\n' >&2
        sed -n '1,160p' "$receiver_stderr" >&2
        exit 6
    }
    attempt=$((attempt + 1))
    [ "$attempt" -lt 100 ] || {
        printf 'fenced-image receiver readiness timed out\n' >&2
        exit 6
    }
    sleep 0.05
done
if ! env -u LD_LIBRARY_PATH -u LD_PRELOAD CLASSPATH="$helper_apk" \
    ...
    printf 'valid fenced-image request failed\n' >&2
...
if env -u LD_LIBRARY_PATH -u LD_PRELOAD CLASSPATH="$helper_apk" \
    /system/bin/app_process -Xnoimage-dex2oat / "$client_class" \
    "$wrong_token" "$wrong_json" unused-e041-socket fenced-image \
    >/dev/null 2> "$wrong_stderr"; then
    printf 'wrong fenced-image capability unexpectedly succeeded\n' >&2
    exit 6
fi
...
python3 - "$wrong_json" "$valid_json" "$receiver_stdout" "$receiver_stderr" \
    "$status_json" "$service_stdout" "$evidence" <<'PY'
import json
import pathlib
import sys
...
assert valid["descriptor_kind"] == "opaque_image_memory_after_producer_fence"
assert valid["producer_fence_completed"] is True
...
assert receiver["gate"] == "E041" and receiver["result"] == "pass"
assert receiver["loader_path"] == "/system/lib64/libvulkan.so"
assert receiver["descriptor_count"] == 1
assert receiver["allocation_size"] == valid["allocation_size"]
assert receiver["memory_type_index"] == valid["memory_type_index"]
assert receiver["width"] == valid["width"] and receiver["height"] == valid["height"]
assert receiver["format"] == valid["format"]
assert receiver["expected_color"] == valid["expected_color"]
assert receiver["mismatched_pixels"] == 0
assert receiver["gpu_wait_elapsed_ns"] >= 0
assert receiver_error_path.stat().st_size == 0
assert "activity_event=11 " in service_path.read_text()
document = {
    "schema_version": 1,
    "gate": "E041",
    "result": "pass",
    "target": "Galaxy Tab S8+ Android Bionic Adreno 730",
    "producer": "visible Android Activity Vulkan image clear and queue-idle fence",
    "consumer": "Termux Bionic Vulkan image import and GPU readback copy",
    "image": "64x64 VK_FORMAT_R8G8B8A8_UNORM optimal-tiling image",
    "cross_uid_transport": "one-time Binder ParcelFileDescriptor then same-UID one-FD SCM_RIGHTS",
    "synchronization": "producer queue idle before handoff; no external semaphore FD",
    "per_frame_java_or_binder": False,
    "wrong_capability": wrong,
    "binder_helper": valid,
    "external_image_import": receiver,
...
print("e041_fenced_external_image=PASS")
PY
printf 'evidence=%s\n' "$evidence"
