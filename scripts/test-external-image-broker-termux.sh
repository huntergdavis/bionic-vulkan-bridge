...
helper_apk="$out_dir/e038-shared-region-client.apk"
receiver="$build_dir/bvb-external-memory-receiver"
control_client="$build_dir/bvb-bridge-client"

for command_name in am aapt cmake cp env grep mktemp od pm python readelf \
    sed sleep tr; do
...
runtime_dir=$(mktemp -d "${TMPDIR:-/tmp}/bvb-e038.XXXXXX")
control_socket="$runtime_dir/bridge.sock"
socket_name="bvb-e038-$(od -An -N8 -tx1 /dev/urandom | tr -d ' \n')"
token=$(od -An -N32 -tx1 /dev/urandom | tr -d ' \n')
wrong_token=ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff
service_stdout="$out_dir/e038-service.stdout"
service_stderr="$out_dir/e038-service.stderr"
receiver_stdout="$out_dir/e038-receiver.stdout"
receiver_stderr="$out_dir/e038-receiver.stderr"
wrong_json="$out_dir/e038-wrong-token.json"
wrong_stderr="$out_dir/e038-wrong-token.stderr"
valid_json="$out_dir/e038-valid-token.json"
valid_stderr="$out_dir/e038-valid-token.stderr"
status_json="$out_dir/e038-activity-status.json"
evidence="$out_dir/e038-external-image-broker.json"
service_pid=
receiver_pid=
cleanup() {
...
while ! grep -q '^bvb-external-memory-receiver: ready ' "$receiver_stdout"; do
    kill -0 "$receiver_pid" 2>/dev/null || {
        printf 'external-image receiver exited before readiness\n' >&2
        sed -n '1,160p' "$receiver_stderr" >&2
        exit 6
    }
    attempt=$((attempt + 1))
    [ "$attempt" -lt 100 ] || {
        printf 'external-image receiver readiness timed out\n' >&2
        exit 6
    }
    sleep 0.05
done
if ! env -u LD_LIBRARY_PATH -u LD_PRELOAD CLASSPATH="$helper_apk" \
    ...
    "$token" "$valid_json" "$socket_name" image \
    >/dev/null 2> "$valid_stderr"; then
    printf 'valid external-image request failed\n' >&2
...
if env -u LD_LIBRARY_PATH -u LD_PRELOAD CLASSPATH="$helper_apk" \
    /system/bin/app_process -Xnoimage-dex2oat / "$client_class" \
    "$wrong_token" "$wrong_json" unused-e038-socket image \
    >/dev/null 2> "$wrong_stderr"; then
    printf 'wrong external-image capability unexpectedly succeeded\n' >&2
    exit 6
fi
...
assert valid["descriptor_kind"] == "opaque_image_memory_plus_sync_fd"
...
assert receiver["gate"] == "E038" and receiver["result"] == "pass"
...
document = {
    ...
    "producer": "visible Android Activity Vulkan image clear",
    "consumer": "Termux Bionic Vulkan image import and GPU readback copy",
    "image": "64x64 VK_FORMAT_R8G8B8A8_UNORM optimal-tiling sampled image",
    "cross_uid_transport": "Binder two ParcelFileDescriptors then LocalSocket two-FD SCM_RIGHTS",
    "synchronization": "producer GPU signal then temporary SYNC_FD consumer GPU wait",
    "steady_state_native_channel": False,
...
print("e038_external_image_broker=PASS")
PY
printf 'evidence=%s\n' "$evidence"
