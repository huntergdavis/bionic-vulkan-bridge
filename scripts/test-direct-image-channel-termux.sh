...
cmake --build "$build_dir" --parallel --target \
    bvb-external-image-direct bvb-bridge-service bvb-bridge-client
for binary in "$build_dir/bvb-external-image-direct" \
...
runtime_dir=$(mktemp -d "${TMPDIR:-/tmp}/bvb-e039.XXXXXX")
...
service_stdout="$out_dir/e039-service.stdout"
service_stderr="$out_dir/e039-service.stderr"
valid_json="$out_dir/e039-valid.json"
valid_stderr="$out_dir/e039-valid.stderr"
wrong_json="$out_dir/e039-wrong-token.json"
wrong_stderr="$out_dir/e039-wrong-token.stderr"
status_json="$out_dir/e039-activity-status.json"
evidence="$out_dir/e039-direct-image-channel.json"
service_pid=
cleanup() {
...
print("e039_direct_image_channel=PASS")
PY
printf 'evidence=%s\n' "$evidence"
