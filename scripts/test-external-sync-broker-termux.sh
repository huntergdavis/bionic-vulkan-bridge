...
helper_apk="$out_dir/e037-shared-region-client.apk"
...
runtime_dir=$(mktemp -d "${TMPDIR:-/tmp}/bvb-e037.XXXXXX")
control_socket="$runtime_dir/bridge.sock"
socket_name="bvb-e037-$(od -An -N8 -tx1 /dev/urandom | tr -d ' \n')"
token=$(od -An -N32 -tx1 /dev/urandom | tr -d ' \n')
wrong_token=ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff
service_stdout="$out_dir/e037-service.stdout"
service_stderr="$out_dir/e037-service.stderr"
receiver_stdout="$out_dir/e037-receiver.stdout"
receiver_stderr="$out_dir/e037-receiver.stderr"
wrong_json="$out_dir/e037-wrong-token.json"
wrong_stderr="$out_dir/e037-wrong-token.stderr"
valid_json="$out_dir/e037-valid-token.json"
valid_stderr="$out_dir/e037-valid-token.stderr"
status_json="$out_dir/e037-activity-status.json"
app_log="$out_dir/e037-visible-host.logcat"
evidence="$out_dir/e037-external-sync-broker.json"
service_pid=
receiver_pid=
cleanup() {
...
while ! grep -q '^bvb-external-memory-receiver: ready ' "$receiver_stdout"; do
    kill -0 "$receiver_pid" 2>/dev/null || {
        printf 'external-sync receiver exited before readiness\n' >&2
        cat "$receiver_stderr" >&2
        exit 6
    }
    attempt=$((attempt + 1))
    [ "$attempt" -lt 100 ] || {
        printf 'receiver readiness timed out\n' >&2
        exit 6
    }
    sleep 0.05
done
if ! env -u LD_LIBRARY_PATH -u LD_PRELOAD CLASSPATH="$helper_apk" \
    ...
    "$token" "$valid_json" "$socket_name" sync \
    >/dev/null 2> "$valid_stderr"; then
    printf 'valid external-sync request failed\n' >&2
    cat "$valid_json" >&2
    cat "$valid_stderr" >&2
    exit 7
fi
...
if env -u LD_LIBRARY_PATH -u LD_PRELOAD CLASSPATH="$helper_apk" \
    /system/bin/app_process -Xnoimage-dex2oat / "$client_class" \
    "$wrong_token" "$wrong_json" unused-e037-socket sync \
    >/dev/null 2> "$wrong_stderr"; then
    printf 'wrong external-sync capability unexpectedly succeeded\n' >&2
    exit 6
fi
...
assert receiver["gate"] == "E037" and receiver["result"] == "pass"
...
assert receiver["expected_fill_word"] == 0xE037C0DE
...
print("e037_external_sync_broker=PASS")
PY
printf 'evidence=%s\n' "$evidence"
