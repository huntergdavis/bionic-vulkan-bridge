...
cleanup() {
    ...
    if [ -n "$runtime_dir" ] && [ -d "$runtime_dir" ] &&
        [ ! -L "$runtime_dir" ]; then
        case "$runtime_dir" in
            "$runtime_parent"/bvb-e047.*) rmdir "$runtime_dir" 2>/dev/null || true ;;
        esac
    fi
}
...
runtime_dir=$(mktemp -d "$runtime_parent/bvb-e047.XXXXXX")
control_socket="$runtime_dir/bridge.sock"
client_stdout="$out_dir/e047-icd-client.stdout"
client_stderr="$out_dir/e047-icd-client.stderr"
service_stdout="$out_dir/e047-service.stdout"
service_stderr="$out_dir/e047-service.stderr"
